...
//   1. tool.execute.before — agent-initiated skill tool calls (source: "skill")
//   2. command.execute.before — user slash-command invocations (source: "command")
...
// name, project directory, source) to skill-usage.log next to this file.
export default {
  id: 'skill-usage',
  server: async (input) => {
    const directory = input?.directory
    const record = (name, source) =>
      appendFileSync(logPath, `${timestamp()}\t${name}\t${directory ?? 'unknown'}\t${source}\n`)
    return {
      config: async (config) => {
      ...
      'tool.execute.before': async (toolInput, output) => {
        ...
        record(output.args?.name ?? 'unknown', 'skill')
      },
